...
public class BleConnectivityService extends Service {
    ...
    private BluetoothGattCharacteristic mZCharacteristic;
    ...
    private BluetoothGattDescriptor mZDescriptor;
    ...
    private boolean mIsNotifyZEnabled;
    private boolean mIsNotifyHumidityEnabled;

    private final IBinder mBinder = new LocalBinder();
    com.example.gymbuddy.service.RepDetector repDetector;
    int reps = 0;
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (mIsNotifyHumidityEnabled) {
                    Log.d(TAG, "humidity gesetzt");
                    mHumidityCharacteristic.setValue(new byte[]{1, 1});
                    gatt.writeCharacteristic(mHumidityCharacteristic);
                }

                if (mIsNotifyZEnabled){
                    Log.d(TAG, "z gesetzt");
                    mZCharacteristic.setValue(new byte[]{1, 1});
                    gatt.writeCharacteristic(mZCharacteristic);
                }
            } else {
    ...
    private void prepareBroadcastAsPerCharacteristicType(BluetoothGattCharacteristic characteristic) {
        ...
        } else if(characteristic.getUuid().equals(UUID.fromString(GattAttributes.Z_CHARACTERISTIC_UUID))){
            broadcastUpdate(Constants.ACTION_Z_AVAILABLE, characteristic);
        }
     ...
     private void broadcastUpdate(final String action, BluetoothGattCharacteristic characteristic) {
        ...
        if (characteristic.getUuid().equals(UUID.fromString(GattAttributes.MANUFACTURER_NAME_CHARACTERISTIC_UUID))) {
            ...
            intent.putExtra(Constants.DATA_MANUFACTURER_MODEL, manufacturerModel);
             ...
             pitch = (characteristic.getIntValue(FORMAT_SINT16,0));


            Log.d(TAG, "broadcastUpdate: Received angular horizontal velocity (pitch ): "+pitch);
            ...
            yaw = (characteristic.getIntValue(FORMAT_SINT16,0));



            Log.d(TAG, "broadcastUpdate: Received angular horizontal velocity (yaw): "+yaw);
        }

        else if (characteristic.getUuid().equals(UUID.fromString(GattAttributes.HUMIDITY_CHARACTERISTIC_UUID))) {
             roll = (characteristic.getIntValue(FORMAT_SINT16, 0));


             Log.d(TAG, "broadcastUpdate: Received angular vertical velocity (y): "+roll);
        ...
        System.out.println("Detected " + detected + " " + repDetector.getActiveEnum() + " " + repDetector.isActive);
         ...
         intent.putExtra(Constants.DATA_Pitch, String.valueOf(reps));
        sendBroadcast(intent);
    }
    ...
    private void readCharacteristicFromService(BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null || characteristic == null) {
            if (mBluetoothAdapter == null){
                Log.e(TAG, "readCharacteristicFromService: BluetoothAdapter not initialized is null");
            }
            if (mBluetoothGatt == null){
                Log.e(TAG, "readCharacteristicFromService: mBluetoothGatt null");
            }
            if (characteristic == null){
                Log.e(TAG, "readCharacteristicFromService: characteristic is null");
            }
    ...
    public void readCharacteristicValue(CharacteristicTypes type) {
        ...
        if (type.equals(CharacteristicTypes.PITCH)) {

            readCharacteristicFromService(mTemperatureCharacteristic);
        } else if (type.equals(CharacteristicTypes.ROLL)) {
            readCharacteristicFromService(mHumidityCharacteristic);
        }else if (type.equals(CharacteristicTypes.Z)){
            readCharacteristicFromService(mZCharacteristic);
        }
    }
    ...
    public void notifyOnCharacteristicChanged(CharacteristicTypes type) {
        ...
        if (type.equals(CharacteristicTypes.PITCH)) {
            ...
            mIsNotifyZEnabled = true;
            notifyCharacteristicFromService(mZDescriptor);
        }
    }
    ...
    private void setupZCharacteristic(BluetoothGatt gatt) {

            Log.d(TAG, String.valueOf(UUID.fromString(GattAttributes.Z_CHARACTERISTIC_UUID)));
            mZCharacteristic = mEnvironmentSensingService
                    .getCharacteristic(UUID.fromString(GattAttributes.Z_CHARACTERISTIC_UUID));
            for (BluetoothGattService service : mEnvironmentSensingService.getIncludedServices()){
                Log.d(TAG, String.valueOf(service.getUuid()));
                Log.d(TAG, String.valueOf(service.getCharacteristics()));

            }
        if (mZCharacteristic != null){
            gatt.setCharacteristicNotification(mZCharacteristic, true);
            mZDescriptor = mZCharacteristic
                    .getDescriptor(UUID.fromString(GattAttributes.CLIENT_CHARACTERISTIC_CONFIG));
            mZDescriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
        }else{
            Log.d(TAG, "mZCharacteristic == null ");
        }
    ...
    public com.example.gymbuddy.service.RepDetector getRepDetector() {
        return repDetector;
    }
